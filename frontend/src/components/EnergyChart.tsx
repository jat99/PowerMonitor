...
export const description = "Energy monitoring chart"
...
export function generateEnergyData(period: TimePeriod): EnergyDataPoint[] {
  const now = new Date()
  ...
  // Calculate cumulative energy - each point shows total energy consumed up to that time
  ...
  return Array.from({ length: dataPoints }, (_, i) => {
    ...
    // Simulate energy increment per period (kWh)
    // Average power ~1.2kW, so per hour = 1.2kWh, per 15min = 0.3kWh
    const baseEnergyIncrement = period === "hour" 
      ? 1.2 / 60 // ~0.02 kWh per minute
      : period === "24hours"
      ? 1.2 / 4 // ~0.3 kWh per 15 min
      : 24 * 1.2 // ~28.8 kWh per day
    
    const timeVariation = Math.sin((hours - 6) * Math.PI / 12) * (baseEnergyIncrement * 0.3)
    const noise = (Math.random() - 0.5) * (baseEnergyIncrement * 0.2)
    const energyIncrement = baseEnergyIncrement + timeVariation + noise
    
    // Accumulate energy (cumulative)
    cumulativeEnergy += energyIncrement
    const energy = Math.round(cumulativeEnergy * 100) / 100
...
export function EnergyChart({ data, title = "Energy Monitor" }: EnergyChartProps) {
  ...
  const chartData = React.useMemo(() => {
    ...
    return generateEnergyData(timePeriod)
  }, [data, timePeriod])
  ...
  const descriptionText = {
    hour: "Last hour of energy data",
    "24hours": "Last 24 hours of energy data",
    week: "Last week of energy data",
  }
